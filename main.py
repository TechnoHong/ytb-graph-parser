...
def get_mr_scraping(url):
    """
    WebScraping 방식(BeautifulSoup, Selenium)으로 mr(Most Replayed) 그래프 getting
    *광고 뜨면 동작 안함
     -> 버림
    :param url: 대상 유튜브 url
    :return: mr graph path 값
    """
    ...
    graph_path = soup.find("path", {"class": "ytp-heat-map-path"}).get('d')
    return graph_path


def get_mr_scraping_mk2(url: str, count: int = 5):
    """
...
print(get_mr_scraping_mk2('https://www.youtube.com/watch?v=B-9I1tDBg60', 10))
